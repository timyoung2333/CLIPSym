# Similar as https://github.com/ahyunSeo/EquiSym/blob/master/dendi_loader.py, under MIT license.
    ...
    def process_theta_ref(self, axis_lbl, axis_coords):
        ...
        if num_lines > 0:
            ...
            theta = np.where(axis_coords[:, 2] == axis_coords[:, 0], 90, \
                            np.arctan(-(axis_coords[:, 3] - axis_coords[:, 1]) / (axis_coords[:, 2] - axis_coords[:, 0] + 1e-15)) \
                                * 180 / np.pi )
...
class NewSymmetryDatasets(NewSymmetryDatasetsBase):
    def __init__(self, train_simple_transform=False, train_flip=False, sym_type='rotation', input_size=(417, 417), get_polygon=2, split='train', root='./sym_datasets/DENDI', \
        ...
        self.train_simple_transform = train_simple_transform
    ...
    def transform_data(self, img, gt, transform, reflection=True, t_resize=None):
        ...
        image_region = torch.tensor([0, 0, 0, 0], dtype=torch.int32)
        if self.split in ['test', 'val', 'all']:
            h, w = img.shape[0], img.shape[1]
            assert self.input_size[0] == self.input_size[1]
            final_size = self.input_size[0]
            scale_ratio = final_size / max(h, w)
            coord1, coord2 = (final_size - min(h, w) * scale_ratio) // 2, (final_size + min(h, w) * scale_ratio) // 2
            if h > w:
                image_region[0], image_region[1] = 0, final_size
                image_region[2], image_region[3] = coord1, coord2
            else:
                image_region[0], image_region[1] = coord1, coord2
                image_region[2], image_region[3] = 0, final_size
        ...
        if self.split in ['test', 'val', 'all'] and t_resize is not None:
            resized_data = t_resize(image=img, axis_gs=axis_gs)
            img, axis_gs = resized_data['image'], resized_data['axis_gs']
        ...
        r_dict = {'img': img, 'mask': mask, 'axis': axis, 'axis_gs': axis_gs, 'img_region': image_region}
    ...
    def __getitem__ (self, index):
        ...
        if self.split in ['test', 'val', 'all']:
            ...
            t_resize = A.Compose([
                ...
                A.PadIfNeeded(min_height=self.input_size[0], min_width=self.input_size[1], \
                            ...
                            #   mask_value=255, originally this one is not saved
                              ),
            ], 
        ...
        if self.sym_type == 'reflection':
            return ref_return['img'], ref_return['mask'], ref_return['axis'], ref_return['axis_gs'], False, ref_a_lbl, ref_return['img_region']
        elif self.sym_type == 'rotation':
            return rot_return['img'], rot_return['mask'], rot_return['axis'], rot_return['axis_gs'], False, rot_a_lbl, rot_return['img_region']
        elif self.sym_type == 'joint':
            ref_return = ref_return['img'], ref_return['mask'], ref_return['axis'], ref_return['axis_gs'], False, ref_a_lbl, ref_return['img_region']
            rot_return = rot_return['img'], rot_return['mask'], rot_return['axis'], rot_return['axis_gs'], False, rot_a_lbl, rot_return['img_region']
