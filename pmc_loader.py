# Loader for SDRW, LDRS and NYU datasets. 
# Mostly based on https://github.com/ahyunSeo/PMCNet/blob/master/dataset.py which is under MIT license.
import os
import pickle
import torch
import pdb
...
class SymmetryDatasets(Dataset):
    def __init__(self, root='./sym_datasets', dataset=['SYM_LDRS'], split='train', resize=False, input_size=[513, 513], angle_interval=45, **kwargs):
        ...
        self.samescale = kwargs.get('samescale', False)
        self.length = 0 
    ...
    def __getitem__(self, index):
        ...
        return img, mask, axis, axis_gs, is_syn, 0, image_region
...
class SYNTHETIC_COCO(Dataset):
    ...
    def get_sym_info (self, rot_ann, h, w, preprocess = False):
        ...
        for i, seg in enumerate(rot_ann):
            ...
            axis = np.logical_and(axis > 0, mask > 0).astype(float)
