# Similar as https://github.com/ahyunSeo/EquiSym/blob/master/model.py, under MIT license.
# Additionally, code under folder `equisym_modeling` are borrowed from the same repository.
...
import numpy as np
import utils
from .equisym_modeling.sync_batchnorm.batchnorm import SynchronizedBatchNorm2d
from .equisym_modeling.module import build_decoder, build_aspp, build_backbone

import pdb
from e2cnn import gspaces
from e2cnn import nn as enn
    
class EquiSym(nn.Module):
    def __init__(self, args=None):
        super(EquiSym, self).__init__()
        self.args = args
        ...
        self.filter = utils.make_gt_filter(5)
        ...
        if self.eq_cnn:
            ...
            gspace_flip = 1
            cutoff_dilation = 0
            from .equisym_modeling.eq_resnet import ReResNet
        ...
        if self.get_theta in [10]:
            ...
            if self.eq_cnn:
                from .equisym_modeling.eq_module import Decoder
        ...
        if self.eq_cnn:
            from .equisym_modeling.eq_module import ASPP
        ...
        if self.eq_cnn:
            if self.get_theta in [10]:
                from .equisym_modeling.eq_module import SingleDecoder
                from .equisym_modeling.eq_resnet import trivial_feature_type
    ...
    def forward(self, img, axis, lbl, a_lbl=None, sym_type='reflection', vis_only=False):
        ### [Step 1]: Base feature extraction with ENC
        feat = self.backbone(img) # (3, 417, 417) -> (2048, 53, 53)
        feat = self.aspp(feat) # (2048, 53, 53) -> (256, 53, 53)

        if self.get_theta in [10]:
            ### [Step 2a]: Apply auxiliary DEC
            theta_out = self.decoder_theta(None, feat) # (256, 53, 53) -> (9, 53, 53)
            ...
            out = self.decoder_axis(theta_sum, theta_out) # (1, 53, 53)
            if not torch.is_tensor(theta_out):
        ...
        axis_out = F.interpolate(out, size=lbl.size()[2:], mode='bilinear', align_corners=True) # (1, 417, 417)
        if vis_only:
        ...
        if self.get_theta:
            ...
            loss += theta_loss * self.args.theta_loss_scale
            losses = (axis_loss, theta_loss)

        return axis_out, loss, losses
