...
class ReResNet(BaseBackbone):
    ...
    def forward(self, x):
        init_x = x # (3, 417, 417) | (3, 640, 480)
        if not self.deep_stem:
            if isinstance(self.conv1, enn.R2Conv):
                x = enn.GeometricTensor(x, self.in_type) # g_tensor (3, 417, 417) | (3, 640, 480)
            x = self.conv1(x) # g_tensor (64, 209, 209) | (64, 320, 240)
            x = self.norm1(x) # g_tensor (64, 209, 209)
            x = self.relu(x)
        x = self.maxpool(x) # g_tensor (64, 105, 105) | (64, 160, 120)
        outs = []
        for i, layer_name in enumerate(self.res_layers):
            res_layer = getattr(self, layer_name)
            x = res_layer(x) # g_tensor (256, 105, 105) -> (512, 53, 53) -> (1024, 53, 53) -> (2048, 53, 53)
            # | (256, 160, 120) -> ... -> (2048, 80, 60)
